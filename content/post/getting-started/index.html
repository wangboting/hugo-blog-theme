...
<body>
  ...
  <div class="section">
    <div class="question">
      <p class="question-text"><font color=white face="楷体">1. 有两个测量量 $A$ 和 $B$，测量误差分别为 $\sigma_A$ 和 $\sigma_B$，它们的相关系数为 $\rho$，问 $f=3A+2B$ 的误差是多少？</font></p>
      <p class="answer">解:根据误差传递公式，可以得到 $f$ 的误差为：
    ...
    \sigma_f = \sqrt{(3\sigma_A)^2 + (2\sigma_B)^2 + 2(3\times 2 \times \rho \sigma_A \sigma_B)}
        ...
        因此，$f=3A+2B$ 的误差是 $\sqrt{9\sigma_A^2 + 4\sigma_B^2 + 12\rho \sigma_A \sigma_B}$。</p>
    <p class="question-text"><font color=white face="楷体">2. 已知概率密度$𝑝(𝑥, 𝑦)$，求概率密度$𝑝(ln(\frac{x}{y}), 𝑙𝑛(𝑥 + 𝑦))$。</font></p>
    <p class="answer">解: 我们可以使用变量替换法来求解这个问题。令 $u = \ln(\frac{x}{y})$ 和 $v = \ln(x + y)$，则有：
        ...
        最后，我们只需要将 $u = \ln(\frac{x}{y})$ 和 $v = \ln(x + y)$ 代入上式即可得到概率密度 $p(\ln(\frac{x}{y}), \ln(x + y))$。</p>
    <p class="question-text"> <font color=white face="楷体">3. $X\sim𝑁(𝜇, 𝜎^2)$，$Y\sim𝑁(𝜇, 𝜎^2)$且 $X $和 $Y$ 相互独立, 求$𝑍_1 = 𝛼𝑋 + 𝛽𝑌$和$𝑍_2 =
        𝛼𝑋 − 𝛽𝑌$的相关系数$(𝛼, 𝛽 ≠ 0)$。</font></p>
        <p class="answer"> 解:根据题意，我们有：
...
<p class="question-text"><font color=white face="楷体">4. $𝑦 = {𝑥^{−1}, 𝑥 > 0;0, 𝑥≤0}$是一个概率密度函数吗，为什么?在不显著改变函数
    在$𝑥 > 0$范围内的主要走向的情况下，如何将它改造成一个概率密度函数，并简单说明改造后的函数为什么是概率密度函数。</font></p>
<p class="answer">解：
    这个函数不是概率密度函数，因为它在$x=0$处不连续，而且对于某些$x$值，它的取值可以大于1。

    要将其改造成概率密度函数，我们需要确保它满足以下两个条件：
    
    <br>1.非负性：对于任何实数$x$，$y(x)>=0$</br>
    <br>2.归一性：$∫𝑦(𝑥)𝑑𝑥=1$</br>
...
<p class="question-text"><font color=white face="楷体">5. 国家天文台刘继峰团队 2019 年在 $3000 $颗 LAMOST 观测的恒星中发现了一颗黑洞双星，问 1)观测到一颗恒星是黑洞双星的概率分布是怎样的?2)刘团队后续又观测了 $12000$ 颗恒星，但是很遗憾没有再找到新的黑洞双星，问此时新观测一颗恒星恰好是黑洞双星的后验概率是多少?</font>
</p>
<p class="answer">解：1) 假设观测到一颗恒星是黑洞双星的概率为p，则观测到一颗不是黑洞双星的概率为1-p。根据题目中的信息，3000颗LAMOST观测的恒星中有一颗是黑洞双星，因此我们可以得到以下方程：
    ...
    因此，在$12001$次观测中，观测到一颗恒星是黑洞双星的后验概率约为$0.00010$。</p>
